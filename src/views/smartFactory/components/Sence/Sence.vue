...
import { ComponentPublicInstance, onMounted, PropType, Ref, ref } from 'vue'
// 导入整个 three.js核心库
...
import Popover from '@/views/smartFactory/components/Popover/Popover.vue'
import { FactoryInfoInterfaceRes } from '@/api/smartFactory/types'
const props = defineProps({
  factoryInfoList: {
    type: Array as PropType<FactoryInfoInterfaceRes[]>,
    default: () => [],
  },
})
  ...
  if (name) {
    ...
    const data = {
      ...
      content: [
        ...
        { label: '噪音分贝', value: factoryInfo?.noise + 'dB' },
        { label: '空气质量', value: factoryInfo?.airQuality },
        { label: '当日产值', value: factoryInfo?.dayOutput },
        { label: '当月产值', value: factoryInfo?.monthOutput },
      ],
    }
    popoverRef.value.setShow(true, factoryInfo ? data : { name })
...
const changeWarningColorByName = (name: string) => {
  const model = rackList.value.find((item) => item.name === name)
  if (model) {
    changeWarningColor(model)
  }
}
// 通过name修改成原始颜色
const changeOriginColorByName = (name: string) => {
  const model = rackList.value.find((item) => item.name === name)
  if (model) {
    changeOriginColor(model)
  }
}
...
// 对外暴露的方法
defineExpose({
  changeWarningColorByName,
  changeOriginColorByName,
})
</script>
